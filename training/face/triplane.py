...
from training.face.networks_stylegan2 import Generator as StyleGAN2Backbone
from training.face.volumetric_rendering.renderer import ImportanceRenderer
from training.face.volumetric_rendering.ray_sampler import RaySampler, AvatarGenRaySampler
from training.face.volumetric_rendering.utils import get_canonical_pose, world2cam_transform
...
class TriPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        canonical_bbox = torch.tensor([[-0.2, 0.032, -0.18], [0.2, 0.4833, 0.18]]).reshape(1, 2, 3).float()
    ...
    def get_canonical_mapping_info(self, c):
        ...
        with torch.no_grad():
            ...
            # Observation Space
            body_model_params = {'body_pose':c_poses[:, 3:66], 'jaw_pose':c_poses[:, 66:69], 'eye_pose':torch.zeros_like(c_poses[:, 0:6]),
            ...
            # import cv2
            # intrinsics = (c_cam[:, 16:25]).reshape(batch_size, 3, 3)
            # intrinsics[:, 0, :] *= 512
            # intrinsics[:, 1, :] *= 512
            # xy = verts_cam / verts_cam[:, :, 2:]
            # xy = torch.einsum('bij,bkj->bki', intrinsics, xy)
            # _img = np.ones((512,512,3), dtype=np.uint8)*255
            # for i, point in enumerate(xy[0, self.FLAME_INDEX]):
            #     _img = cv2.circle(_img, (int(point[0]), int(point[1])), 1, (255, 0, 0), 1)
            ...
            face_min = torch.amin(verts_cam[:, self.FLAME_INDEX], dim=1)
            face_max = torch.amax(verts_cam[:, self.FLAME_INDEX], dim=1)
            ...
            #     'body_bbox': body_bbox,
            #     'ober2cano_transform': ober2cano_transform[:, self.FLAME_INDEX],
            #     'verts': verts[:, self.FLAME_INDEX],
            #     'verts_canonical': verts_canonical[:, self.FLAME_INDEX],
            #     'faces': self.faces,
            #     'triangles': triangles,
            #     'lbs_weights': self.lbs_weights[self.FLAME_INDEX],
            ...
            # ax.scatter(verts_canonical[0, self.FLAME_INDEX, 0].detach().cpu(), verts_canonical[0, self.FLAME_INDEX, 1].detach().cpu(), verts_canonical[0, self.FLAME_INDEX, 2].detach().cpu())
...
from training.face.networks_stylegan2 import FullyConnectedLayer
