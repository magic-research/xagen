...
import torch
from training.hand.networks_stylegan2 import Conv2dLayer, SynthesisLayer, ToRGBLayer
from torch_utils.ops import upfirdn2d
from torch_utils import persistence
from torch_utils import misc

from training.hand.networks_stylegan2 import SynthesisBlock
import numpy as np
from training.hand.networks_stylegan3 import SynthesisLayer as AFSynthesisLayer
...
class SuperresolutionHybrid4XDC(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
                ...
                **block_kwargs):
        super().__init__()
        assert img_resolution[0] == 256

        use_fp16 = sr_num_fp16_res > 0
        self.input_resolution = 64
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlock(channels, 128, w_dim=512, resolution=128,
                img_channels=3, is_last=False, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.block1 = SynthesisBlock(128, 64, w_dim=512, resolution=256,
                img_channels=3, is_last=True, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
...
class SynthesisBlockNoUp(torch.nn.Module):
    def __init__(self,
        ...
        fused_modconv_default   = True,         # Default value of fused_modconv. 'inference_only' = True for inference, False for training.hand.
