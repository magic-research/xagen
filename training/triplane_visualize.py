...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def synthesis(self, ws, ws_geo, c_body, c_face, c_hand, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, return_eikonal=False, smplx_reg=False, **synthesis_kwargs):
        ...
        out = {
            ...
            'verts_body': canonical_mapping_kwargs_body['verts_cam'],
            'verts_face': canonical_mapping_kwargs_face['verts_cam'],
            'verts_hand': canonical_mapping_kwargs_hand['verts_cam'],
            'verts_transformed': canonical_mapping_kwargs_hand['verts_canonical'],
            'vert_indices_body': canonical_mapping_kwargs_body['vert_indices'],
            'vert_indices_face': canonical_mapping_kwargs_face['vert_indices'],
            'vert_indices_hand': canonical_mapping_kwargs_hand['vert_indices'],
            'verts_canonical': canonical_mapping_kwargs_body['verts_canonical'],
        }
    ...
    def sample_mixed(self, coordinates, directions, ws, c_body, c_face, c_hand, canonical_mapping_kwargs, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        # Same as sample, but expects latent vectors 'ws_face' instead of Gaussian noise 'z'
        planes = self.backbone.synthesis(ws, update_emas=update_emas, **synthesis_kwargs)
        body_planes = planes[:, :32*3].reshape(len(planes), 3, 32, planes.shape[-2], planes.shape[-1])
        face_planes = planes[:, 32*3:40*3].reshape(len(planes), 3, 32, planes.shape[-2]//2, planes.shape[-1]//2)
        hand_planes = planes[:, 40*3:].reshape(len(planes), 3, 32, planes.shape[-2]//2, planes.shape[-1]//2)
        self.renderer_body.plane_axes = self.renderer_body.plane_axes.to(coordinates.device)
        return self.renderer_body.run_model(body_planes, face_planes, hand_planes, self.decoder, self.decoder, self.decoder, coordinates, directions, self.rendering_kwargs, return_sdf=True, return_eikonal=False, **canonical_mapping_kwargs)
