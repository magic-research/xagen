...
class SuperresolutionHybrid4X(torch.nn.Module):
    ...
    def forward(self, rgb, x, ws, ws_face, **block_kwargs):
        ...
        x, rgb = self.block0(x, rgb, ws, inplace=False, **block_kwargs)
        x, rgb = self.block1(x, rgb, ws, inplace=False, **block_kwargs)
        return rgb
...
#                                                   mode='bilinear', align_corners=False, antialias=self.sr_antialias) # -- Not available in PyTorch 1.10.0
...
class SuperresolutionHybrid2X(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
        ...
        self.input_resolution = 256
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlockNoUp(channels, 128, w_dim=w_dim, resolution=[256, 128],
                img_channels=3, is_last=False, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.block1 = SynthesisBlock(128, 64, w_dim=w_dim, resolution=[512, 256],
                img_channels=3, is_last=True, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1, 3, 3, 1]))
    ...
    def forward(self, rgb, x, ws, **block_kwargs):
        ws = ws[:, -1:, :].repeat(1, 3, 1)
        # ws_face = ws_face[:, -1:, :].repeat(1, 3, 1)
        # ws_hand = ws_hand[:, -1:, :].repeat(1, 3, 1)
        # ws = torch.cat([ws, ws_face, ws_hand], dim=-1)
