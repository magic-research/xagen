...
class ImportanceRenderer(torch.nn.Module):
    ...
    def forward(
        ...
        offset_network=None, canonical_reg=False, smplx_reg_full_space=False):
        ...
        if use_cam_dist:
            ...
            rendering_options['box_warp'] = 1.0
        else:
        ...
        # ax.scatter(sample_coordinates[0, :, 0].cpu(), sample_coordinates[0, :, 1].cpu(), sample_coordinates[0, :, 2].cpu())
        # ax.scatter(ray_origins[0, :, 0].cpu(), ray_origins[0, :, 1].cpu(), ray_origins[0, :, 2].cpu())
        ...
        if False:
            out = self.run_model(planes, decoder, sample_coordinates, sample_directions, rendering_options, return_eikonal=return_eikonal, return_sdf=return_sdf, offset_network=offset_network, smplx_reg_full_space=smplx_reg_full_space, **canonical_mapping_kwargs)
            eikonal_coarse = out['eikonal']
        else:
            out = self.run_model(planes, decoder, sample_coordinates, sample_directions, rendering_options, offset_network=offset_network, smplx_reg_full_space=smplx_reg_full_space, **canonical_mapping_kwargs)
            eikonal_coarse = None
        ...
        # if smplx_reg and smplx_sdf is None and not smplx_reg_full_space:
        ...
        return rgb_final, depth_final, weights.sum(2), eikonal, smplx_sdf, sdf_reg_weights, sdf_coarse
    ...
    def run_model(
        ...
        smplx_params=None, smplx_reg_full_space=False):
        ...
        #     offset_network=offset_network, canonical_reg=canonical_reg, smplx_params=smplx_params)
        sample_coordinates_cano = sample_coordinates

        sampled_features = sample_from_planes(self.plane_axes, planes, sample_coordinates_cano, canonical_bbox, padding_mode='zeros', box_warp=options['box_warp'])
        # sampled_features = sampled_features.reshape(sampled_features.shape[0], 3, num_pts, -1)
        ...
        if smplx_reg_full_space:
            # verts_dis = torch.clamp(torch.sqrt(body_sdf.reshape(*out['sdf'].shape)**2), 1e-2, 1e3)
    ...
    def deformation(self, offset_network, coords, smplx_params=None, bbox_norm=None):
        deformed_coords = offset_network(coords, smplx_params, bbox_norm)
    ...
    def canonical_mapping(
        ...
        if not canonical_reg:
            ...
            if use_deformation:
                ...
                bbox_norm = torch.norm(body_bbox[:, 1]-body_bbox[:, 0], dim=-1)
                filtered_coords_offset = self.deformation(offset_network, filtered_coords, smplx_params, bbox_norm)
        ...
        # filtered_coords_cano, valid = self.unpose(coords, verts, ober2cano_transform, lbs_weights, device)
        # fig = plt.figure()
        # ax = fig.add_subplot(projection='3d')
    ...
    def filter_and_pad(coords, bbox, coords_ori=None, bbox_ori=None):
        ...
        # bbox = torch.tensor([[-0.1389, 0.032, -0.1719], [0.1393, 0.4833, 0.1628]]).reshape(1,2,3).float().cuda()
        device = coords.device
