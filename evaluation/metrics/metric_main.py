    ...
    for key, values in list(results.items()):
        if opts.num_gpus > 1:
            if isinstance(values, tuple):
                ...
                value = torch.as_tensor(values, dtype=torch.float64, device=opts.device)
                torch.distributed.broadcast(tensor=value, src=0)
                value = float(value.cpu())
                results[key] = value
...
def fid50k_full(opts):
    ...
    fids = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=50000)
    return dict(fid50k_full=fids)
