...
from training.hand.networks_stylegan2 import DiscriminatorBlock, MappingNetwork, DiscriminatorEpilogue
...
class DualDiscriminator(torch.nn.Module):
    def __init__(self,
        ...
        **kwargs,
        ...
        if c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        if right_hand_pose_dim > 0:
            self.mapping_smplx = MappingNetwork(z_dim=0, c_dim=right_hand_pose_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)
    ...
    def forward(self, image, image_raw, c, update_emas=False, **block_kwargs):
        ...
        cam = c[:, :self.c_dim]
        # smplx = c[:, self.c_dim:275]
        smplx_poses = c[:, 275:]
        right_hand_pose = smplx_poses[:, 114:]
        ...
        if self.smplx_dim > 0:
            # if self.disc_c_noise > 0: cam += torch.randn_like(cam) * cam.std(0) * self.disc_c_noise
            smplx_map = self.mapping_smplx(None, right_hand_pose)
