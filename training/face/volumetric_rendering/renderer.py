...
from training.face.volumetric_rendering.ray_marcher import MipRayMarcher2
from training.face.volumetric_rendering import math_utils
from training.face.volumetric_rendering.utils import grid_sample
...
class ImportanceRenderer(torch.nn.Module):
    ...
    def run_model(self, planes, decoder, sample_coordinates, sample_directions, options,
        verts=None, verts_canonical=None, faces=None, obs_bbox=None, canonical_face_bbox=None, canonical_body_bbox=None, triangles=None, 
        ...
        sample_coordinates_cano, base_sdf, mask, coordinates_mask, max_length, deformation_reg = self.canonical_mapping(
            sample_coordinates, verts, verts_canonical, faces, obs_bbox, canonical_face_bbox, canonical_body_bbox, 
        ...
        sampled_features = sample_from_planes(self.plane_axes, planes, sample_coordinates_cano, canonical_face_bbox, padding_mode='zeros', box_warp=options['box_warp'], obs_bbox=obs_bbox)
    ...
    def canonical_mapping(
        self, coords, verts, verts_canonical, faces, obs_bbox, canonical_face_bbox, canonical_body_bbox, 
        ...
        if not density_reg:
            ...
            if use_deformation:
                ...
                filtered_coords_cano, coords_cano_offset = self.deformation(offset_network, filtered_coords_cano, filtered_coords, ws, ws_geo, smplx_params, canonical_face_bbox)
                deformation_reg = coords_cano_offset.norm(dim=-1).mean()
            
            filtered_coords = filtered_coords_cano
            
            # mask, canonical_filtered_coords_mask, max_length, filtered_coords, filtered_coords_ori = self.filter_and_pad(filtered_coords, canonical_face_bbox.repeat(coords.shape[0], 1, 1))    
        ...
        #         self.filter_and_pad(filtered_coords, canonical_face_bbox.repeat(coords.shape[0], 1, 1), filtered_coords_ori, obs_bbox)
        # else:
