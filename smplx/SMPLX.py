...
LEFT_HAND_INDEX = [20, 37, 38, 39, 133, 25, 26, 27, 128, 28, 29, 30, 129, 34, 35, 36, 131, 31, 32, 33, 130]
RIGHT_HAND_INDEX = [21, 52, 53, 54, 144, 40, 41, 42, 139, 43, 44, 45, 140, 49, 50, 51, 142, 46, 47, 48, 141]
...
class SMPLX(nn.Module):
    ...
    """
    def __init__(self, 
                 smplx_model_path="smplx/assets/SMPLX_NEUTRAL_2020.npz", 
                 n_shape=200, 
                 n_exp=50, 
                 extra_joint_path="smplx/assets/smplx_extra_joints.yaml", 
                 j14_regressor_path="smplx/assets/SMPLX_to_J14.pkl"):
    ...
    def forward(self, shape_params=None, expression_params=None,
                    ...
                    left_hand_pose=None, right_hand_pose=None, pose2rot=False,
                    full_pose=None, return_A=False, return_transform=False, return_offsets=False):
        """
            Args:
        ...
        if global_pose is None:
            if not pose2rot:
                ...
                global_pose = torch.zeros([3], dtype=self.global_pose.dtype, device=self.global_pose.device).unsqueeze(0).expand(batch_size, -1)
        ...
        outputs = (vertices, landmarks, joints)
        if return_A:
            outputs = outputs + (A, )
        if return_transform:
            outputs = outputs + (T, )
        if return_offsets:
            outputs = outputs + (shape_offsets, pose_offsets)
    ...
    def get_canonical_pose(self, use_da_pose=False):
        body_pose = self.body_pose.clone()
        if use_da_pose:
            body_pose[0, ...] = torch.tensor([[[0.8776, -0.4794,  0.0000],
                                            [0.4794,  0.8776,  0.0000],
                                            [0.0000,  0.0000,  1.0000]]])
            body_pose[1, ...] = torch.tensor([[[0.8776,  0.4794,  0.0000],
                                        [-0.4794,  0.8776, -0.0000],
                                        [-0.0000,  0.0000,  1.0000]]])
        full_pose = torch.cat([self.global_pose, body_pose,
                                self.jaw_pose, self.eye_pose,
                                self.left_hand_pose, self.right_hand_pose], dim=0)
        return full_pose